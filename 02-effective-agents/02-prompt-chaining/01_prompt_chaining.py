...
OUTLINER_SYSTEM_PROMPT = (
    ...
    "that will serve as section headings for a blog post. "
    "Focus on STRUCTURE: each heading should target a specific, actionable aspect of the topic. "
    "Avoid vague umbrella headings. "
    "Bad: 'Understanding X: What It Is and Why It Matters' "
    "Good: 'Why X fails silently and how to catch it' "
    "Output only the title on the first line, then the 5 bullet points. No extra commentary."
)
OUTLINER_USER_PROMPT = "Create a blog outline for: {topic}"

WRITER_SYSTEM_PROMPT = (
    "You are a technical blog writer targeting senior developers. Given an outline "
    "(title + bullet points), write a concise blog post. Use the title as an H1 heading "
    "and each bullet point as an H2 section. Write 1-2 short paragraphs per section. "
    "For EVERY section: include at least one concrete example, code snippet, or specific "
    "data point from web search. Take clear stances — avoid 'it depends' hedging. "
    "Write as if you've built this yourself and are sharing hard-won lessons. "
    "No filler phrases like 'In today's world...' or 'It's important to note...'. "
    "IMPORTANT: When you use information from web search, reference the source inline "
    "using markdown links, e.g. [according to the Anthropic docs](https://url). "
    "Every claim or data point should be traceable to a source. "
    "Aim for under 1000 words. Always use web search for current, accurate information."
)
WRITER_USER_PROMPT = "Write a full blog post from this outline:\n\n{outline}"

EDITOR_SYSTEM_PROMPT = (
    "You are a senior technical editor. Review and improve the blog post: "
    "1) Remove generic filler sentences that could apply to any technology. "
    "2) Ensure every section has at least one specific example or data point. "
    "3) Sharpen vague claims into concrete statements. "
    "4) Add a '## Key Takeaways' section with 3-5 actionable bullet points "
    "(each should tell the reader exactly what to do, not just what to know). "
...
class PromptChain:
    ...
    def run(self, topic: str, on_event: ChainCallback | None = None) -> str:
        ...
        # Append sources from web search to the final post
        if searches:
            sources_section = "\n\n## Sources\n\n" + "\n".join(
                f"- [{s['title']}]({s['url']})" for s in searches
            )
    ...
    def on_chain_event(event: str, data: dict[str, Any]) -> None:
        ...
        if event == "step_start":
            ...
            if data.get("searches"):
                ...
                console.print(
                    Panel("\n".join(lines), title=f"{data['name']} Sources", border_style="dim")
                )
